...
INPUT_FILENAME = "london_drone_Short.mp4"
OUTPUT_FILENAME = "output_london_drone_Short.mp4"
...
def download_video():
    ...
    if os.path.exists(INPUT_FILENAME):
        os.remove(INPUT_FILENAME) # Clean start
        
    print(f"⬇ Downloading video from YouTube...")
    
    ydl_opts = {
        'format': 'best[ext=mp4]',  # Get best quality MP4
        'outtmpl': INPUT_FILENAME,  # Save as specific filename
        'quiet': True,
        'no_warnings': True
    }
    ...
    try:
        ...
        print(f"✅ Download complete: {INPUT_FILENAME}")
        return True
...
def process_video():
    """Runs inference on the downloaded video"""
    # 1. Load Model
